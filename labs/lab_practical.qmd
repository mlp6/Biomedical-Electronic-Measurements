...
Not everyone can present during the dedicated 3 hour time block that would have been our final exam slot (Dec 12, 09:00-12:00), so I would greatly appreciate folks signing up for days in advance!
...
![Wheatstone Bridge with Thermistor](./wheatstone/wheatstone.svg){fig-align="center" width="400"}
...
- Include in your plot of the output voltage how the power supplies by the source varies as this thermistor resistance changes.
...
when the filtered output voltage exceeds a certain threshold voltage.  This
comparator will output a `LOW` signal (0 V) when the temperature is below the
threshold and a `HIGH` signal (+5 V) when the temperature exceeds the threshold.
